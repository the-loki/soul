...
use crate::param::{QueryParam, Term};
...
where
    P: QueryParam,
{
    pub fn each(&self, mut f: impl for<'row> FnMut(P::Item<'row>)) {
        // SAFETY: self.raw is a live query created for self.world.
        ...
        while unsafe { sys::soul_ecs_query_next(iter) } {
            ...
            for row in 0..count {
                ...
                let _guards = P::borrow_row(self.world, entity, &self.terms);
